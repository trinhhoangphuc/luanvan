...
class NhapController extends Controller
{
    public function index($id)
    {
    	try{
    		...
    		$nhap = DB::select("
                ...
                ORDER BY (DATEDIFF(NOW(), n_hanSD)) DESC"
            );
     ...
     public function storeProduct(Request $request, $id)
    {
        try
        {
            $nhap = Nhap::where("sp_ma", $id)->where("n_ngaySX", $request->get("ngaysanxuat"))->where("n_hanSD", $request->get("hansudung"))->where("hv_ma", $request->get("maHuong"))->where("n_soLuong", ">", 0)->first();
            if(!$nhap)
            {
                $nhap = new Nhap();
                $nhap->sp_ma = $id;
