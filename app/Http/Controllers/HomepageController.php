...
use App\Banner, App\Sanpham, App\Hinhanh, App\Huongvi, App\Chitiethuong, App\Danhgia, App\Loai, App\Hang, App\Khachhang, App\Donhang, App\Nhap, App\Vanchuyen, App\Thanhtoan, App\Chitiethoadon, DB, Session, Cart;
...
class HomepageController extends Controller
{
   ...
   public function postRegister(Request $request)
    ...
    public function addToCart($id){
        ...
        if($sanpham){
            $nhap = Nhap::select("nhap.*", "huongvi.hv_ten")->join("huongvi", "huongvi.hv_ma", "nhap.hv_ma")->where("sp_ma", $id)->where('n_soLuong', ">", 0)->orderBy("n_hanSD", "asc")->first();
            if($nhap){
                ...
                Cart::add(array('id'=>$nhap->n_ma, 'name'=>$sanpham->sp_ten, 'qty'=>1, 'price'=>$price, 'options' => array('img' => $sanpham->sp_hinh, 'sp_ma'=>$sanpham->sp_ma, 'price_2'=>$sanpham->sp_giaBan,'discount'=>$sanpham->sp_giamGia, "taste"=>$nhap->hv_ten)));
                return response(['error'=>false, "message"=>true], 200);
            }else 
     ...
     public function addToCart_2($id, $qty, $mahuong){
        $sanpham = Sanpham::where("sp_ma", $id)->first();
        if($sanpham){
            $nhap = Nhap::select("nhap.*", "huongvi.hv_ten")->join("huongvi", "huongvi.hv_ma", "nhap.hv_ma")->where("sp_ma", $id)->where("nhap.hv_ma", $mahuong)->where('n_soLuong', ">=", $qty)->orderBy("n_hanSD", "asc")->first();
            if($nhap){
                if($sanpham->sp_giamGia > 0) $price = $sanpham->sp_giamGia;
                else  $price = $sanpham->sp_giaBan;
                Cart::add(array('id'=>$nhap->n_ma, 'name'=>$sanpham->sp_ten, 'qty'=>$qty, 'price'=>$price, 'options' => array('img' => $sanpham->sp_hinh, 'sp_ma'=>$sanpham->sp_ma, 'price_2'=>$sanpham->sp_giaBan,'discount'=>$sanpham->sp_giamGia, "taste"=>$nhap->hv_ten)));
                return response(['error'=>false, "message"=>true], 200);
            }else 
                return response(["error"=>false, "message"=>false], 200);
    ...
    public function getCart(){
        ...
        return view("customer.giohang", compact("cart", "total"));
    }
            ...
            if(Session::has("customer_id")){
                ...
                if($donhang->save()){
                    foreach (Cart::content() as $key) {
                        ...
                        $chitiethoadon->sp_ma = $key->options->sp_ma;
